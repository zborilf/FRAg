...
%	check_used(TUSEDMEANS, INTENDEDMEANS).
  ...
  get_plan(can_reasoning, event( _ , _ , _ , _ , _ , _ , USEDMEANS),
	   [ _ | TINTENDEDMEANS], Intended_Means):-
	format(atom(STRING), "[CANDBG] ~w", [USEDMEANS]),
	println_debug(STRING, candbg),
	check_used(USEDMEANS, Intended_Means),
	get_plan(can_reasoning, _ , TINTENDEDMEANS, Intended_Means).

  get_plan(can_reasoning, _, [Intended_Means| _ ], Intended_Means).
