...
% Supporting library that checks whether new eposode occurs
% It occurs when some agent is registered twice in a list (agents_perceived)
% TODO, this version does not distinguish between different environments and 
% their instances, if there is a new eposode in an instance, it deletes 
% everything.
% History stores the history of all perceptions, but probably unnecessarily
