...
% Some statistical calculations for the implementation of the environment.
% Poisson and normal distribution etc.
...
poisson_dist_sample2(Events, _, S, X, Events2):-
    S>X,
    Events2 is Events -1.

poisson_dist_sample2(N, Lambda, S, X, Events):-
    poisson(Lambda, N, X2),
    S2 is S+X2,
    N2 is N+1,
    poisson_dist_sample2(N2, Lambda, S2, X, Events).
                     
poisson_dist_sample(Lambda, Events):-
    random(0.0, 1.0, X),
    poisson_dist_sample2(0, Lambda, 0, X, Events).
