...
mcts_rollouts(Program, Steps, Simulations, Results, Rewards_Path, 
    ...
    virtual_name(Virtual_Agent),     % ?? reset_clones, load_all_instances??
...
open_engine_file(Agent, Agent_Loop, 0):-
  ...
  %  file_directory_name(FRAg_Path, Directory),
    ...
    get_time(Timestamp),                % Z¡sk n¡ aktu ln¡ho timestampu
    format_time(atom(FormattedTime), '%d.%m.%Y %H:%M:%S', Timestamp), % Form tov n¡
    writeln(FormattedTime).             % Vìstup na konzoli
