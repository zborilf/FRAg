...
%            pocet akci v pytli je potom heuristika pro tuto sadu programu
  ...
  heuristicDo([H|T],BAG,BAGOUT):-
	expandBag(H,BAG,BAGEXP),
	append(BAG,BAGEXP,BAGOUT2),
	heuristicDo(T,BAGOUT2,BAGOUT).
...
random_action(X,ACTION):-random(1,X,ACTIONNUMBER),
		random_action2(ACTIONNUMBER,ACTION).

random_intention2(ACTN,ITNN,INTNL,INTNL,[]).

random_intention2(ACTN,ITNN,ITNL2,ITNL,[act(ACTION,ITNN,ITNL2)|T]):-
		random_action(ACTN,ACTION),
		ITNL3 is ITNL2 + 1,
		random_intention2(ACTN,ITNN,ITNL3,ITNL,T).

random_intention(ACTN,X,ITNN,ITN):-  % X je max delka intesny
                random(1,X,ITNL),
		ITNL2 is ITNL + 1,
		random_intention2(ACTN,ITNN,1,ITNL2,ITN).
...
print_intention([],0).
print_intention([act(ACTION,_,_)],1):-format("do(~w).",[ACTION]).
print_intention([act(ACTION,_,_)|T],N):-format("do(~w);",[ACTION]),print_intention(T,N2),N is N2+1.

print_intentions(_,N,[]):-format("// size=~w~n",[N]),pokus(P),assert(result(P,N,0,0)).
print_intentions(I,N,[H|T]):-random_action2(I,G),I2 is I+1,format("!~w.~n+!~w<- ",[G,G]),print_intention(H,N2),nl,N3 is N+N2, print_intentions(I2,N3,T).
...
go(ACTN,AVGLNG,ITNN):-         
	...
	pokus(P),
	format(atom(OUT),"res/out~w.asl",[P]),
	tell(OUT),
...
%	ITNSOUT = [
%	[act(a,1,1),act(b,1,2),act(c,1,3),act(d,1,4),act(e,1,5),act(f,1,6),act(g,1,7),act(h,1,8)],
%	[act(b,2,1),act(a,2,2),act(a,2,3),act(d,2,4),act(b,2,5),act(x,2,6)],
%	[act(d,3,1),act(b,3,2),act(c,3,3),act(c,3,4),act(a,3,5)],
%	[act(c,4,1),act(a,4,2),act(d,4,3),act(a,4,4),act(w,4,5),act(x,4,6)],
%	[act(z,5,1),act(a,5,2),act(c,5,3),act(e,5,4),act(b,5,5)],
%	[act(c,6,1),act(g,6,2),act(b,6,3),act(a,6,4),act(a,6,5)]
%     	[act(h,7,1),act(x,7,2),act(b,7,3),act(a,7,4)]
%	],
