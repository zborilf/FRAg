...
mcts_rollouts(Program, Steps, 0, Results, Rewards_Path, Expanded, FinalResult):-
    ...
    %engine_yield(runResult(results(Rewards_Path, Results), 0, Expanded,	 Goals_Remain)),
    !,
    garbage_all,
    ...
    %mcts_rollouts(Program, Steps, Simulations, [], Rewards_Path, Expanded).
    FinalResult = runResult(results(Rewards_Path, Results), 0, Expanded, Goals_Remain).
...
mcts_rollouts(Program, Steps, Simulations, Results, Rewards_Path,
    ...
    mcts_rollouts(Program, Steps, Simulations2, [Rewards| Results],
                          Rewards_Path, Expanded, FinalResult).
...
% SIMULATIONS (MCTS ALGORITHM)STARTS HERE
    ...
    get_intention(_, Intentions, Intention),

    %convert silent_program into string
    term_string(Silent_Program, Silent_Program_String),
    term_string(Intentions, Intentions_String),
    %call python implementation with converted silent_program
    py_func('program_mcts', get_program(Silent_Program_String, Expansions, Simulations, Intentions_String), Ret),
