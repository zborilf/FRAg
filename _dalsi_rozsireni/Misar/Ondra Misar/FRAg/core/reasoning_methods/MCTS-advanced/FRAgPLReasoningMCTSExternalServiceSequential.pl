...
reasoning_method(mcts_reasoning_external_service_sequential).
...
%!  update_model(mcts_reasoning_external_service_sequential) is nondet
...
update_model(mcts_reasoning_external_service_sequential):-
...
% SIMULATIONS (MCTS ALGORITHM)STARTS HERE
    ...
    py_func('mcts_service_call', get_result(Silent_Program_String, Facts_String, Expansions, Simulations, Simulation_Steps, 'basic_sequential', Environment, Absolute_Module_Name), Ret),
...
%!  get_intention(mcts_reasoning_external_service_sequential, +Intentions, -Intention) is det
%
...
get_intention(mcts_reasoning_external_service_sequential, Intentions,
              intention(Intention_ID, Content, active)):-
...
get_substitution(mcts_reasoning_external_service_sequential, Action, Contexts, Vars, Context_Out):-
...
%!  get_plan(mcts_reasoning_external_service_sequential, +Event, +Means, -Intended_Means) is det
...
get_plan(mcts_reasoning_external_service_sequential, Event, Means, [IM_Plan, IM_Context]):-
...
get_plan(mcts_reasoning_external_service_sequential, _, _, []).





%!  init_reasoning(mcts_reasoning_external_service_sequential) is det
...
init_reasoning(mcts_reasoning_external_service_sequential):-
