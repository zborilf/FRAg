...
reasoning_method(mcts_reasoning_online_learning).


%    Simulations - rollouts for Online learning variant of MCTS
%==============================================================================
mcts_rollouts_learning(Steps, FinalResult):-
    retractall(insert_state_action( _, _ )),
    ...
    findall(insert_state_action(P, I), insert_state_action(P, I), State_Action_List),
    retractall(state_action( _, _ , _)),
    FinalResult = runResult(Rewards, State_Action_List).
...
%!  update_model(mcts_reasoning_online_learning) is nondet
...
update_model(mcts_reasoning_online_learning):-
...
% SIMULATIONS (MCTS ALGORITHM)STARTS HERE
    ...
    py_func('mcts_service_call', get_result(Silent_Program_String, Facts_String, Expansions, Simulations, Simulation_Steps, 'learning', Environment, Absolute_Module_Name), Ret),
...
%!  get_intention(mcts_reasoning_online_learning, +Intentions, -Intention) is det
%
...
get_intention(mcts_reasoning_online_learning, Intentions,
              intention(Intention_ID, Content, active)):-
...
get_substitution(mcts_reasoning_online_learning, Action, Contexts, Vars, Context_Out):-
...
%!  get_plan(mcts_reasoning_online_learning, +Event, +Means, -Intended_Means) is det
...
get_plan(mcts_reasoning_online_learning, Event, Means, [IM_Plan, IM_Context]):-
...
get_plan(mcts_reasoning_online_learning, _, _, []).





%!  init_reasoning(mcts_reasoning_online_learning) is det
...
init_reasoning(mcts_reasoning_online_learning):-
