...
reasoning_method(mcts_reasoning_external_service_parallel).


%    Simulations - rollouts for External service architecture
%==============================================================================
mcts_rollouts_call_service(Steps, FinalResult):-
...
%!  update_model(mcts_reasoning_external_service_parallel) is nondet
...
update_model(mcts_reasoning_external_service_parallel):-
...
% SIMULATIONS (MCTS ALGORITHM)STARTS HERE
    ...
    py_func('mcts_service_call', get_result(Silent_Program_String, Facts_String, Expansions, Simulations, Simulation_Steps, 'basic_parallel', Environment, Absolute_Module_Name), Ret),
...
%!  get_intention(mcts_reasoning_external_service_parallel, +Intentions, -Intention) is det
%
...
get_intention(mcts_reasoning_external_service_parallel, Intentions,
              intention(Intention_ID, Content, active)):-
...
get_substitution(mcts_reasoning_external_service_parallel, Action, Contexts, Vars, Context_Out):-
...
%!  get_plan(mcts_reasoning_external_service_parallel, +Event, +Means, -Intended_Means) is det
...
get_plan(mcts_reasoning_external_service_parallel, Event, Means, [IM_Plan, IM_Context]):-
...
get_plan(mcts_reasoning_external_service_parallel, _, _, []).





%!  init_reasoning(mcts_reasoning_external_service_parallel) is det
...
init_reasoning(mcts_reasoning_external_service_parallel):-
