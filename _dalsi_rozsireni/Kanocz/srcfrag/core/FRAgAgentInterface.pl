...
%!  situate_agent(+Agent, +Environment) is det
%   Entry‐point for (re)‐situating an agent; serializes via a mutex.
situate_agent(Agent, Environment) :-
    % Debug: entry
    format("[DBG] enter situate_agent(~w, ~w)~n", [Agent, Environment]),
    with_mutex(frag_mutex,
       situate_agent2(Agent, Environment)).


%!  situate_agent2(+Agent, +Environment) is det
%   1) already in that environment
situate_agent2(Agent, Environment) :-
    agent_environment(Agent, Environment, Environment),
    format("[DBG] branch: already situated~n", []),
    format("[IFC] Agent ~w is already situated in environment ~w~n",
           [Agent, Environment]).

%!  situate_agent2(+Agent, +Environment) is det
%   2) environment exists, so assert and call its add_agent/1
situate_agent2(Agent, Environment) :-
    format("[DBG] branch: new environment, checking existence~n", []),
    environment(Environment),                % does it exist?
    !,                                       % commit if it does
    format("[DBG] environment(~w) holds, asserting agent_environment/3~n",
           [Environment]),
    assertz(agent_environment(Agent, Environment, Environment)),
    % build and call: miconic(add_agent, Agent)  or shop(add_agent, Agent)
    SituateGoal =.. [Environment, add_agent, Agent],
    format("[DBG] calling ~w~n", [SituateGoal]),
    call(SituateGoal).

%!  situate_agent2(+Agent, +Environment) is det
%   3) fallback: no such environment
situate_agent2(Agent, Environment) :-
    format("[DBG] branch: environment(~w) does NOT hold~n", [Environment]),
...
agent_acts(Agent, Environment, Act, Result):-
    format("[DBG] agent_acts: ~w dispatching ~w to ~w~n",
