...
set_environment(Environment) :-
    thread_self(Agent),
    % 1) entry log
    format("[DBG] ~w: enter set_environment(~w)~n", [Agent, Environment]),
    % 2) attempt to situate, catching any exception
    (   catch(
            fRAgAgentInterface:situate_agent(Agent, Environment),
            Err,
            ( format("[ERROR] ~w: exception in situate_agent: ~w~n", [Agent, Err]),
              fail
            )
        )
    ->  % 3a) success
        format(atom(SuccessMsg), "[SYSDBG] Agent ~w successfully situated to environment ~w~n",
               [Agent, Environment]),
        println_debug(SuccessMsg, systemdbg),
        format("[DBG] ~w: set_environment(~w) succeeded~n", [Agent, Environment])
    ;   % 3b) failure
        format("[ERROR] Agent ~w: set_environment(~w) failed~n", [Agent, Environment]),
        println_debug(
          format(atom(FailMsg), "[ERROR] Environment '~w' initialization failed", [Environment]),
          error
        ),
        % fail so callers know it didn’t work
        fail
    ).
...
fa_init_environments:-
    bagof(Environment, default_environment(Environment), Environments),
    !,
    list_to_set(Environments, UniqueEnvs),
    fa_init_environments2(UniqueEnvs).
...
fa_init_environments2([]) :-
    format("~n[fine] All environments processed.~n", []).

fa_init_environments2([Env|Envs]) :-
    % show us exactly what Env is
    format(">>> Initializing environment: ~w~n", [Env]),
    (   set_environment(Env)
    ->  format("    [ ok ]   '~w' initialized successfully.~n", [Env])
    ;   format("[ERROR] Environment '~w' initialization failed.~n", [Env])
    ),
    fa_init_environments2(Envs).
