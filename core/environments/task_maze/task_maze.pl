...
init_tasks:-                  
   tasks(Tasks),
   add_facts(task_maze, [tasks(Tasks)]).                                             


get_percepts_doors(Coords, Doors):-
   findall(door(Direction), dirs(Coords, Coords2, Direction), Doors), 
  writeln(hotovo(Doors)).
...
get_percepts_position(Position, Agent, Percepts):-
    findall_environment(task_maze, Agent, item(Position, Item, Reward), 
			Items),
    findall(door(Direction), dirs(Position, _, Direction), Doors),
...
init_beliefs_agent(Agent):-
    ...
    get_percepts_position(Position, Agent, Percepts),
    add_beliefs_agents([Agent], [tasks(Tasks)]),
