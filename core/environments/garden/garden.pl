...
:- discontiguous garden:garden/2.
:- discontiguous garden:garden/3.
:- discontiguous garden:garden/4.
...
update_critical_zones(Agent) :-
    env_utils:findall_environment(garden, Agent, zone_moisture(Zone, _), ZMs),
    sort(2, @=<, ZMs, SortedZMs),
    extract_two_zones(SortedZMs, WorstZones),
    env_utils:findall_environment(garden, Agent, critical_zone( _ ), OldCrs),
    ( OldCrs \= [] ->
