...
expandBag([act(A,_,_)|T], Bag,T2):-
		member(act(A,B,C), Bag),        % bag ma odpovidajici akci
		delete(Bag, act(A,B,C), Bag2),  % pouzili jsme z bagu odpovidajici akci, priste uz nemuzeme
		expandBag(T, Bag2, T2).		% a T2 je vysledek, akce H nerozsiri bag, ma odpovidajici zde

expandBag([H|T], Bag, [H|T2]):-          % v Bagu nebyla odpovidajici, takze tato bude expandovat bag a je ve tretim termu
		expandBag(T, Bag, T2).   


heuristicDo([],L,L).

heuristicDo([H|T], Bag, Bag_Out):-
		expandBag(H, Bag, BAGEXP),
		append(Bag, BAGEXP,BAGOUT2),
		heuristicDo(T,BAGOUT2, Bag_Out).

heuristic([H|T],N):-heuristicDo(T,H,LOUT),
		    length(LOUT,N).
...
%
% vytvori hada, had2 od zadaneho indexu a zbyvajicih intensen jako vyse, ale 
% vezme prvniho a pokracuje v seznamu navazani, nez bude vysledek nula, nebo 
% seznam navazani prazdny
%
...
% zde mame jako druhy term jen jednu intensnu, nezanorujeme, vytvorime hada 
% smerem k posledni, neviditelne intensne jako dvojici (N a kam ukazuje N do I2)
...
%  napr [[sb(1,3,2,1),sb(2,1,3,0)],[sb(1,5,2,2),sb(2,2,3,0)]] jsou dva hadi,
%  kteri usetri dve akce (maji oba delku dva, takze v kazdem jedna akce se 
%  zredukuje)
%  analyza - suma delek prvku minus pocet prvku
%  pripadne nahradime nejlepsiho hada, ktery je ulozen v dynamickem predikaty 
%  best_snake(had,fitnes hada).
...
hadi_poradi(L):-             
	retractall(perm(X)),
	h_permutace(L, Permutation),
	retractall(count(_)),
	assert(count(1)),
	hadi_poradi2(Permutation).
...
%%%%%%%%%%%%%%%%%%%%%%%
%         Heuristiky, vyberu nejdelsi z bindings pro kazdou intensnu
...
% vyhodnoti posloupnost intensen, druhy term je seznam nejlepsich bindingsu 
% [bind(1,2,[]),bind(1,3,[]),bind(1,4,[p(2,3),p(3,4),p(4,6)]),
% bind(2,3,[p(1,1)]),bind(2,4,[]),bind(3,4,[])]
...
best_permutace(IDXS,BESTBINDS):-                               
		bagof(bind(N1,N2,P),bind(N1,N2,P),BINDINGSY),
		h_permutace(IDXS,PERMS),
		evaluate_permutations(PERMS,BINDINGSY,BESTP,BESTPV),
		format("% Snake solution ~w / ~w ~n",[BESTP,BESTPV]),
		hadi_propojeni(BESTP,BESTBINDS).
...
go(Index, Remains, NO_Variables, Avg_Plan_Length, NO_Intentions):-   
	retractall(pokus(_)),
	assert(pokus(Index)),
	go(NO_Variables, Avg_Plan_Length, NO_Intentions),
	printResult(Index),
	Index2 is Index+1,
	Remains2 is Remains - 1,
	told,!,
	go(Index2, Remains2, NO_Variables, Avg_Plan_Length, NO_Intentions).		
